...
export default function GalleryPage() {
  ...
  // The first-run welcome popup (mounted app-wide) now collects a name and
  // picture, so the gallery no longer force-redirects a new visitor to the full
  // profile — they can browse straight away.
      ...
      <main className="mx-auto max-w-[1600px] px-6 pb-24">
        <section className="py-14 text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-3.5 py-1.5 text-xs font-medium text-zinc-400">
            ...
            {t("{templates} templates · {palettes} palettes · one channel profile", { templates: TEMPLATES.length, palettes: PALETTES.length })}
          </span>
          <h1 className="mx-auto mt-6 max-w-3xl text-balance text-5xl font-bold leading-[1.05] tracking-tight md:text-6xl">
            <span className="gradient-text">{t("Professional stream overlays")}</span>
            <br />
            <span className="text-white">{t("in under five minutes.")}</span>
          </h1>
          <p className="mx-auto mt-5 max-w-xl text-balance text-[15px] leading-relaxed text-zinc-400">
            {t("Everything is free, every template works as a still or animated. Fill in your channel details once — every design picks up your name, logo and socials automatically.")}
          </p>
        </section>
        ...
        <div className="sticky top-16 z-30 -mx-6 mb-8 border-y border-white/[0.06] bg-ink-950/85 px-6 py-4 backdrop-blur-xl">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button onClick={openBlank} title={t("Open an empty canvas and build from scratch")}>
            ...
            />
            <Button onClick={() => importRef.current?.click()} title={t("Import a design file (.asarayja-design.json)")}>
            ...
            <div className="w-40">
              ...
              >
                ...
                <option value="gothic">{t("Gothic")}</option>
                <option value="pride">{t("Pride")}</option>
              </Select>
            </div>
            ...
            <button
              ...
              title={t("Preview every template using the colours from your channel profile")}
              className={cx(
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            {tags.length > 0 && <Chip onClick={() => { setTags([]); resetPaging(); }}>{t("Clear")} ×</Chip>}
          </div>
          ...
          {(collection === "gothic" || collection === "pride") && (
            <div className="mt-3 flex flex-wrap items-center gap-1.5">
              <span className="mr-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-600">
                {collection === "gothic" ? t("Gothic style") : t("Pride style")}
              </span>
        ...
        {shown.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/10 py-24 text-center">
            <p className="text-sm text-zinc-400">{t("No templates match those filters.")}</p>
        ...
        {visible < filtered.length && (
          <div className="mt-14 text-center">
            <Button variant="outline" onClick={() => setVisible((v) => v + PAGE_SIZE)}>
              {t("Load {count} more", { count: Math.min(PAGE_SIZE, filtered.length - visible) })}
            </Button>
          </div>
        )}
      </main>
