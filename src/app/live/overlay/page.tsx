...
/**
 * The OBS browser source. The overlay code travels in the query string
 * (?code=…); the payload itself rides in the `#d=` fragment (the only thing
 * OBS's isolated Chromium can read).
 *
 * Resolution order:
 *  1. `#d=` fragment — works inside OBS, which has its own empty localStorage.
...
function LiveOverlay() {
  ...
  // the settled still frame instead of a running clock. The clock is unbounded
  // so entry animations play once and hold while ambient presets run forever.
