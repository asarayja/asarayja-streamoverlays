import type { CoreTheme, Palette, StyleTag, Theme } from "@/lib/types";
import { completeTheme } from "@/lib/theme";
import { CUSTOM_PALETTES } from "@/data/custom-designs";

/**
 * Shipping palettes. Authors tune the eight core tokens (plus any overrides);
 * `completeTheme` derives the other eight, so every palette exposes the full
 * sixteen-token system. Swapping a palette recolours every layer of every
 * template in one assignment.
 *
 * `tags` feed the gallery filter — a template variant inherits its palette's
 * tags on top of its own structural tags.
 */
interface PaletteDef extends Omit<Palette, "theme"> {
  theme: CoreTheme & Partial<Theme>;
}

const DEFS: PaletteDef[] = [
  {
    id: "purple-neon",
    name: "Purple Neon",
    collection: "core",
    tags: ["Purple", "Neon", "Dark", "Cyberpunk"],
    theme: {
      warning: "#edcd82",
      success: "#35d492",
      primary: "#8b5cf6",
      secondary: "#22d3ee",
      accent: "#e879f9",
      background: "#0d0716",
      text: "#ece6f7",
      border: "#a78bfa",
      glow: "#c084fc",
      shadow: "#000000",
    },
  },
  {
    id: "blue-neon",
    name: "Blue Neon",
    collection: "core",
    tags: ["Blue", "Neon", "Dark", "Sci-Fi"],
    theme: {
      primary: "#3b82f6",
      secondary: "#06b6d4",
      accent: "#60a5fa",
      background: "#050b18",
      text: "#e6edf8",
      border: "#38bdf8",
      glow: "#22d3ee",
      shadow: "#000000",
    },
  },
  {
    id: "pink-vapor",
    name: "Pink Vapor",
    collection: "core",
    tags: ["Pink", "Neon", "Anime", "Cyberpunk"],
    theme: {
      error: "#e05c3e",
      primary: "#ec4899",
      secondary: "#a855f7",
      accent: "#22d3ee",
      background: "#150a18",
      text: "#f5eaf3",
      border: "#f472b6",
      glow: "#f0abfc",
      shadow: "#000000",
    },
  },
  {
    id: "green-emerald",
    name: "Green Emerald",
    collection: "core",
    tags: ["Green", "Neon", "Dark", "Esports"],
    theme: {
      success: "#a3eec9",
      primary: "#10b981",
      secondary: "#34d399",
      accent: "#a3e635",
      background: "#04140e",
      text: "#eef3f0",
      border: "#31493d",
      glow: "#6ee7b7",
      shadow: "#000000",
    },
  },
  {
    id: "orange-fire",
    name: "Orange Fire",
    collection: "core",
    tags: ["Orange", "Esports", "Dark", "RGB"],
    theme: {
      primary: "#f97316",
      secondary: "#e2be69",
      accent: "#e05656",
      background: "#160a04",
      text: "#f6efe7",
      border: "#453a30",
      glow: "#fdba74",
      shadow: "#000000",
    },
  },
  {
    id: "dark-gray",
    name: "Dark Gray",
    collection: "core",
    tags: ["Dark", "Minimal"],
    theme: {
      warning: "#e8d6a6",
      primary: "#52525b",
      secondary: "#a1a1aa",
      accent: "#e4e4e7",
      background: "#09090b",
      text: "#fafafa",
      border: "#3f3f46",
      glow: "#eaeaef",
      shadow: "#000000",
    },
  },
  // Spec palette "Minimal": white background, light-gray surface, slate,
  // steel blue, royal blue, near-black text.
  {
    id: "white-clean",
    name: "Minimal",
    collection: "core",
    tags: ["Light", "Minimal", "Cozy"],
    theme: {
      warning: "#92400e",
      primary: "#475569",
      secondary: "#5b7c99",
      accent: "#2563eb",
      background: "#fbfbfc",
      surface: "#eff1f4",
      surfaceSecondary: "#e4e7ec",
      text: "#0f1115",
      border: "#b3bac7",
      glow: "#93b4f5",
      shadow: "#7d8593",
    },
  },
  {
    id: "gold",
    name: "Gold",
    collection: "core",
    tags: ["Fantasy", "Dark", "Orange"],
    theme: {
      warning: "#fb923c",
      primary: "#d4af37",
      secondary: "#8b6f1f",
      // A violet accent — the complement of gold — so accent-coloured text pops
      // against the gold ribbons/panels (and reads on the dark ground) instead
      // of vanishing gold-on-gold.
      accent: "#7c3aed",
      background: "#120e05",
      text: "#fffbeb",
      border: "#574b21",
      glow: "#fcd34d",
      shadow: "#000000",
    },
  },
  {
    id: "silver",
    name: "Silver",
    collection: "core",
    tags: ["Minimal", "Sci-Fi", "Light"],
    theme: {
      primary: "#94a3b8",
      secondary: "#cbd5e1",
      accent: "#f1f5f9",
      background: "#0a0f1c",
      text: "#f8fafc",
      border: "#3a4150",
      glow: "#e2e8f0",
      shadow: "#000000",
    },
  },
  {
    id: "cyber-blue",
    name: "Cyber Blue",
    collection: "core",
    tags: ["Cyberpunk", "Blue", "Sci-Fi", "Neon"],
    theme: {
      primary: "#00e5ff",
      secondary: "#e170bf",
      accent: "#e3df63",
      background: "#030a18",
      text: "#e0fbff",
      border: "#266673",
      glow: "#22d3ee",
      shadow: "#000000",
    },
  },
  {
    id: "crimson",
    name: "Crimson",
    collection: "core",
    tags: ["Red", "Horror", "Dark", "Esports"],
    theme: {
      error: "#ef6a3d",
      primary: "#dc2626",
      secondary: "#7f1d1d",
      accent: "#f87171",
      background: "#170708",
      text: "#fef2f2",
      border: "#ef4444",
      glow: "#f87171",
      shadow: "#000000",
    },
  },
  {
    id: "ice",
    name: "Ice",
    collection: "core",
    tags: ["Nordic", "Blue", "Cozy", "Light"],
    theme: {
      accentSecondary: "#6c82ea",
      primary: "#7dd3fc",
      secondary: "#e0f2fe",
      accent: "#0ea5e9",
      background: "#081420",
      text: "#f0f9ff",
      border: "#3a5570",
      glow: "#e0f2fe",
      shadow: "#0b0d11",
    },
  },
  // Spec palette "Fantasy": deep forest, dark moss, emerald, gold, magic blue,
  // warm ivory.
  {
    id: "forest",
    name: "Fantasy",
    collection: "core",
    tags: ["Green", "Cozy", "Fantasy", "Nordic"],
    theme: {
      primary: "#2fa36b",
      secondary: "#d9a23c",
      accent: "#4fa3e8",
      background: "#0c1710",
      surface: "#18261c",
      text: "#f5efdc",
      border: "#4a6a55",
      glow: "#6fd3a0",
      shadow: "#000000",
    },
  },
  {
    id: "sunset",
    name: "Sunset",
    collection: "core",
    tags: ["Orange", "Pink", "Cozy", "Anime"],
    theme: {
      warning: "#c28a18",
      surface: "#2e1e28",
      error: "#ce3348",
      accentSecondary: "#fde68a",
      primary: "#ec8b99",
      secondary: "#e8a266",
      accent: "#fcd34d",
      background: "#1a0a14",
      text: "#f9eedd",
      border: "#5d414c",
      glow: "#fdba74",
      shadow: "#000000",
    },
  },
  {
    id: "galaxy",
    name: "Galaxy",
    collection: "core",
    tags: ["Purple", "Fantasy", "Sci-Fi", "Dark"],
    theme: {
      surface: "#201f36",
      success: "#2fbf7a",
      error: "#db5e6b",
      accentSecondary: "#90c8ea",
      primary: "#7c7ee8",
      secondary: "#d946ef",
      accent: "#38bdf8",
      background: "#07061a",
      text: "#e7e4f6",
      border: "#474a6b",
      glow: "#c4b5fd",
      shadow: "#000000",
    },
  },

  /* --------------------- Vivid — punchy, saturated ----------------------- */
  {
    id: "toxic-lime",
    name: "Toxic Lime",
    collection: "core",
    tags: ["Green", "Neon", "Dark", "RGB"],
    theme: {
      // Acid lime family on a neutral near-black so it pops without the green
      // washing into the background.
      primary: "#8dff2e", secondary: "#00e5a0", accent: "#d4ff00",
      background: "#0b0d10", text: "#f5ffe9", border: "#8dff2e", glow: "#a8ff45", shadow: "#000000",
      success: "#35d492", warning: "#ffe14d", error: "#ff5252",
    },
  },
  {
    id: "magenta-punch",
    name: "Magenta Punch",
    collection: "core",
    tags: ["Pink", "Neon", "Dark", "Cyberpunk"],
    theme: {
      primary: "#ff2e97", secondary: "#b026ff", accent: "#ff6fd8",
      background: "#10040e", text: "#ffe9f6", border: "#ff2e97", glow: "#ff5fb0", shadow: "#000000",
    },
  },
  {
    id: "electric-teal",
    name: "Electric Teal",
    collection: "core",
    tags: ["Blue", "Neon", "Dark", "Sci-Fi"],
    theme: {
      primary: "#00e5d0", secondary: "#2ea8ff", accent: "#5cffe1",
      background: "#03100f", text: "#e4fffb", border: "#00e5d0", glow: "#38ffe8", shadow: "#000000",
    },
  },
  {
    id: "ruby",
    name: "Ruby",
    collection: "core",
    tags: ["Red", "Esports", "Dark"],
    theme: {
      primary: "#ff1e3c", secondary: "#ff7a00", accent: "#ff5470",
      background: "#120306", text: "#ffe6ea", border: "#ff1e3c", glow: "#ff4d66", shadow: "#000000",
    },
  },
  {
    id: "amethyst",
    name: "Amethyst",
    collection: "core",
    tags: ["Purple", "Fantasy", "Dark"],
    theme: {
      primary: "#9d4edd", secondary: "#c77dff", accent: "#e0aaff",
      background: "#0b0616", text: "#f1e7ff", border: "#9d4edd", glow: "#b983ff", shadow: "#000000",
    },
  },
  {
    id: "tangerine",
    name: "Tangerine",
    collection: "core",
    tags: ["Orange", "Neon", "Dark"],
    theme: {
      primary: "#ff8a00", secondary: "#ff3d00", accent: "#ffc300",
      background: "#120a02", text: "#fff1de", border: "#ff8a00", glow: "#ffa733", shadow: "#000000",
    },
  },
  {
    id: "aqua-volt",
    name: "Aqua Volt",
    collection: "core",
    tags: ["Green", "Blue", "Neon", "RGB"],
    theme: {
      primary: "#00ffc6", secondary: "#00b4ff", accent: "#b6ff00",
      background: "#04100c", text: "#e6fff7", border: "#00ffc6", glow: "#4dffd8", shadow: "#000000",
    },
  },
  {
    id: "ultraviolet",
    name: "Ultraviolet",
    collection: "core",
    tags: ["Purple", "Blue", "Neon", "Cyberpunk"],
    theme: {
      primary: "#6c2bff", secondary: "#00d4ff", accent: "#b026ff",
      background: "#070518", text: "#ece9ff", border: "#6c2bff", glow: "#9a5cff", shadow: "#000000",
    },
  },
  {
    id: "coral-neon",
    name: "Coral Neon",
    collection: "core",
    tags: ["Pink", "Orange", "Neon"],
    theme: {
      primary: "#ff5e5b", secondary: "#ff9f1c", accent: "#ff87ab",
      background: "#120705", text: "#ffebe8", border: "#ff5e5b", glow: "#ff7d7a", shadow: "#000000",
    },
  },
  {
    id: "sapphire",
    name: "Sapphire",
    collection: "core",
    tags: ["Blue", "Esports", "Dark"],
    theme: {
      primary: "#2979ff", secondary: "#00e5ff", accent: "#536dfe",
      background: "#030818", text: "#e6eeff", border: "#2979ff", glow: "#5c93ff", shadow: "#000000",
    },
  },
  {
    id: "lime-soda",
    name: "Lime Soda",
    collection: "core",
    tags: ["Green", "Neon", "RGB"],
    theme: {
      primary: "#b4ff00", secondary: "#00e5a0", accent: "#eeff00",
      background: "#06100a", text: "#f2ffe0", border: "#b4ff00", glow: "#c9ff4d", shadow: "#000000",
    },
  },
  {
    id: "hot-pink",
    name: "Hot Pink",
    collection: "core",
    tags: ["Pink", "Neon", "Anime"],
    theme: {
      primary: "#ff3ea5", secondary: "#ff77e9", accent: "#ffd1f0",
      background: "#12040c", text: "#ffe9f5", border: "#ff3ea5", glow: "#ff66bd", shadow: "#000000",
    },
  },
  {
    id: "inferno",
    name: "Inferno",
    collection: "core",
    tags: ["Orange", "Red", "Neon"],
    theme: {
      primary: "#ff3d00", secondary: "#ffc400", accent: "#ff7a00",
      background: "#100402", text: "#ffeedb", border: "#ff3d00", glow: "#ff6a26", shadow: "#000000",
    },
  },
  {
    id: "emerald-neon",
    name: "Emerald Neon",
    collection: "core",
    tags: ["Green", "Neon", "Esports"],
    theme: {
      primary: "#00e676", secondary: "#ffd54a", accent: "#1de9b6",
      background: "#041008", text: "#e6ffef", border: "#00e676", glow: "#4dff9e", shadow: "#000000",
    },
  },
  {
    id: "cyber-pink",
    name: "Cyber Pink",
    collection: "core",
    tags: ["Pink", "Blue", "Cyberpunk", "RGB"],
    theme: {
      // Hot pink + cyan cyberpunk — the accent stays in the pink family so it
      // doesn't clash as a lone purple.
      primary: "#ff2e97", secondary: "#00e5ff", accent: "#ff6fd8",
      background: "#0a0812", text: "#ffe9f6", border: "#ff2e97", glow: "#ff5fb0", shadow: "#000000",
    },
  },


  /* ------------------------------- Gothic -------------------------------- */
  // The ten launch packs. Each is a complete Theme, so a gothic template can
  // swap between them with one click, exactly like the core palettes.
  // Spec palette "Dark Goth": charcoal black, gunmetal, deep burgundy, royal
  // purple, antique silver, ivory white, dark crimson glow.
  {
    id: "midnight-cathedral",
    subStyle: "Dark Goth",
    name: "Midnight Cathedral",
    collection: "gothic",
    tags: ["Dark", "Purple", "Fantasy"],
    theme: {
      warning: "#d9bf83",
      primary: "#571c30",
      secondary: "#4b2a6e",
      accent: "#c6c2ce",
      background: "#0b0b0e",
      surface: "#232129",
      text: "#edeaf2",
      border: "#46404f",
      glow: "#8a1e3c",
      shadow: "#000000",
    },
  },
  {
    id: "pastel-nightmare",
    subStyle: "Pastel Goth",
    name: "Pastel Nightmare",
    collection: "gothic",
    tags: ["Pink", "Purple", "Dark"],
    // Spec palette "Pastel Goth": dark plum, charcoal violet, soft pink,
    // lavender, mint, warm white, pastel purple glow.
    theme: {
      warning: "#ebce85",
      success: "#55c08a",
      surface: "#241c2e",
      primary: "#E7A5D7",
      secondary: "#B6A5E8",
      accent: "#9FE0D0",
      background: "#140f19",
      text: "#f5e9f1",
      border: "#5e4d71",
      glow: "#D793EB",
      shadow: "#0B080E",
    },
  },
  {
    id: "crimson-vampire",
    subStyle: "Vampire Goth",
    name: "Crimson Vampire",
    collection: "gothic",
    tags: ["Red", "Dark", "Horror"],
    theme: {
      warning: "#e6c25f",
      success: "#3fae7c",
      error: "#de7443",
      accentSecondary: "#e4778e",
      primary: "#5E102A",
      secondary: "#24142F",
      accent: "#C93055",
      background: "#0b0a0f",
      text: "#F1E8ED",
      border: "#71616D",
      glow: "#7A1638",
      shadow: "#000000",
    },
  },
  // Tuned to "Haunting Whispers" (piktochart.com/blog/gothic-color-palette):
  // a muted violet ramp #1F1A2A -> #B8A3C1.
  {
    id: "moonlit-witch",
    subStyle: "Witch Goth",
    name: "Moonlit Witch",
    collection: "gothic",
    tags: ["Purple", "Dark", "Fantasy"],
    theme: {
      warning: "#dccfb2",
      primary: "#5B4B8A",
      secondary: "#7A6A9A",
      accent: "#B8A3C1",
      background: "#14101C",
      surface: "#241B30",
      text: "#F3EFFA",
      border: "#6F5F8E",
      glow: "#9c7bd5",
      shadow: "#000000",
    },
  },
  {
    id: "victorian-mourning",
    subStyle: "Victorian Goth",
    name: "Victorian Mourning",
    collection: "gothic",
    tags: ["Dark", "Orange", "Fantasy"],
    // Surface leans on the warm dark wood of "Enchanted Forest" (piktochart).
    theme: {
      warning: "#ebd8ad",
      accentSecondary: "#9f8041",
      surface: "#241C16",
      primary: "#6E1F2E",
      secondary: "#3A2C22",
      accent: "#C8A44D",
      background: "#0D0A08",
      text: "#F2E9DC",
      border: "#8A7355",
      glow: "#D9B45C",
      shadow: "#000000",
    },
  },
  {
    id: "cyber-coven",
    subStyle: "Cyber Goth",
    name: "Cyber Coven",
    collection: "gothic",
    tags: ["Cyberpunk", "Purple", "Neon", "Dark"],
    // Spec palette "Cyber Goth": near black, graphite, electric purple, neon
    // cyan, acid green, white, electric blue glow.
    theme: {
      primary: "#9b30ff",
      secondary: "#23e8da",
      accent: "#b8e62e",
      accentSecondary: "#f02eae",
      background: "#0b0614",
      surface: "#1a1a1f",
      text: "#f7f4fb",
      border: "#8c4fd0",
      glow: "#3d9bff",
      shadow: "#000000",
    },
  },
  {
    id: "haunted-garden",
    subStyle: "Gothic Fantasy",
    name: "Haunted Garden",
    collection: "gothic",
    tags: ["Green", "Dark", "Fantasy"],
    theme: {
      success: "#3fae83",
      error: "#c9744f",
      primary: "#547257",
      secondary: "#8E4152",
      accent: "#B9CDA0",
      background: "#0c110d",
      text: "#EDF2E6",
      border: "#3a4c3d",
      glow: "#a7df9b",
      shadow: "#000000",
    },
  },
  // Tuned to "Dark Elegance" (piktochart gothic palettes): slate navy with a
  // vivid rose-red pair and petal-pink highlights.
  {
    id: "gothic-romance",
    subStyle: "Romantic Goth",
    name: "Gothic Romance",
    collection: "gothic",
    tags: ["Red", "Pink", "Dark"],
    theme: {
      success: "#35b87f",
      error: "#e36a3d",
      primary: "#df3a60",
      secondary: "#8D99AE",
      accent: "#F9AFAF",
      accentSecondary: "#EF233C",
      background: "#0f1119",
      surface: "#23263A",
      text: "#f6e9ed",
      border: "#4d5366",
      glow: "#f35a6d",
      shadow: "#000000",
    },
  },
  {
    id: "raven-manor",
    subStyle: "Gothic Horror",
    name: "Raven Manor",
    collection: "gothic",
    tags: ["Blue", "Dark", "Nordic"],
    // Navy depths from "Phantom's Embrace" (piktochart), silver kept as the
    // signature accent, its crimson repurposed as the secondary accent.
    theme: {
      success: "#49a67e",
      error: "#d0703f",
      primary: "#4c6ea9",
      secondary: "#223052",
      accent: "#B9C4D6",
      accentSecondary: "#E94560",
      background: "#10121a",
      surface: "#1B2233",
      text: "#F2F5F9",
      border: "#55677F",
      glow: "#aac7ec",
      shadow: "#000000",
    },
  },
  {
    id: "candy-coffin",
    subStyle: "Pastel Goth",
    name: "Candy Coffin",
    collection: "gothic",
    tags: ["Pink", "Green", "Dark"],
    theme: {
      error: "#e27565",
      primary: "#F5A9C8",
      secondary: "#A9E5D2",
      accent: "#C3A9F0",
      background: "#141018",
      text: "#f3e7ee",
      border: "#5d3c4f",
      glow: "#F0B9DF",
      shadow: "#0B070D",
    },
  },

  // Requested palette: red, purple and near-black, with a bone-white accent so
  // bats, webs and ghosts read as white against the night.
  {
    id: "crimson-hollow",
    name: "Crimson Hollow",
    collection: "core",
    tags: ["Red", "Purple", "Dark", "Horror"],
    // Red, purple and near-black. The secondary is a warm red-violet, not the
    // blue-violet it used to be — the fog reads purple, in family with the
    // crimson, rather than drifting cold and blue.
    theme: {
      primary: "#a8123a",
      secondary: "#853088",
      accent: "#efeaf6",
      accentSecondary: "#c94f7c",
      background: "#0d0910",
      surface: "#1b1522",
      text: "#f5f0f8",
      border: "#6a4a6b",
      glow: "#c81e4e",
      shadow: "#000000",
    },
  },
  // A pastel-goth sibling: a dusk-plum night (a touch lighter than near-black)
  // with soft pink, lavender and mint. The background stays dark enough that
  // the graveyard still silhouettes and the pastel moon, bats and webs read
  // against it — pastel colours, not a light theme.
  {
    id: "pastel-hollow",
    name: "Pastel Hollow",
    collection: "core",
    tags: ["Pink", "Purple", "Dark", "Cozy"],
    theme: {
      primary: "#e89ac4",
      secondary: "#a99ae0",
      accent: "#e0d3f2",
      accentSecondary: "#a9e5d2",
      background: "#150f1c",
      surface: "#241a30",
      text: "#f8f2fb",
      border: "#6a5a80",
      glow: "#c9a0e8",
      shadow: "#000000",
    },
  },

  // A blood-red sibling: the same near-black night, but red-dominant with a
  // bone-white accent so bats, webs and ghosts stay white against it.
  {
    id: "blood-moon",
    name: "Blood Moon",
    collection: "core",
    tags: ["Red", "Dark", "Horror"],
    theme: {
      primary: "#b31229",
      secondary: "#7a1f3a",
      accent: "#f3e4e6",
      accentSecondary: "#e0556a",
      background: "#11090a",
      surface: "#231517",
      text: "#f8eeef",
      border: "#6e4548",
      glow: "#e01e34",
      shadow: "#000000",
    },
  },

  /* -------- Light-background pastels (alongside the dark pastel set) ------ */
  // Light-blue daybreak sky, deeper blue for text and accents.
  {
    id: "daybreak",
    name: "Daybreak",
    collection: "core",
    tags: ["Blue", "Light", "Cozy"],
    theme: {
      primary: "#2f7fc6",
      secondary: "#66a2d6",
      accent: "#1f6bad",
      background: "#e2eef8",
      surface: "#cedfee",
      surfaceSecondary: "#bad0e3",
      text: "#0f202e",
      textSecondary: "#3a6388",
      border: "#8baac6",
      glow: "#6aace7",
      shadow: "#99a9b8",
      success: "#278657",
      warning: "#957323",
      error: "#ba2c37",
    },
  },
  // Soft pink cotton-candy ground with rose accents.
  {
    id: "cotton-candy",
    name: "Cotton Candy",
    collection: "core",
    tags: ["Pink", "Light", "Anime"],
    theme: {
      primary: "#c62f61",
      secondary: "#d6668b",
      accent: "#ad1f4e",
      background: "#f8e2ea",
      surface: "#eeced9",
      surfaceSecondary: "#e3bac8",
      text: "#2e0f19",
      textSecondary: "#8f3d58",
      border: "#c68b9e",
      glow: "#e76a93",
      shadow: "#b899a3",
      success: "#268254",
      warning: "#916f22",
      error: "#ba2c37",
    },
  },
  // Pale mint sea-glass with deep teal accents.
  {
    id: "sea-glass",
    name: "Sea Glass",
    collection: "core",
    tags: ["Green", "Light", "Cozy"],
    theme: {
      primary: "#2fc68f",
      secondary: "#66d6ad",
      accent: "#198f64",
      background: "#e2f8f0",
      surface: "#ceeee2",
      surfaceSecondary: "#bae3d4",
      text: "#0f2e23",
      textSecondary: "#31725a",
      border: "#8bc6b0",
      glow: "#6ae7b9",
      shadow: "#99b8ac",
      success: "#298e5c",
      warning: "#9d7925",
      error: "#ba2c37",
    },
  },
  // Light lilac wisteria with plum accents.
  {
    id: "wisteria",
    name: "Wisteria",
    collection: "core",
    tags: ["Purple", "Light", "Anime"],
    theme: {
      primary: "#662fc6",
      secondary: "#8f66d6",
      accent: "#531fad",
      background: "#eae2f8",
      surface: "#daceee",
      surfaceSecondary: "#c9bae3",
      text: "#1b0f2e",
      textSecondary: "#5b3d8f",
      border: "#a08bc6",
      glow: "#986ae7",
      shadow: "#a499b8",
      success: "#257e52",
      warning: "#8c6c21",
      error: "#ba2c37",
    },
  },
  // Warm peach-cream ground with terracotta accents.
  {
    id: "peach-cream",
    name: "Peach Cream",
    collection: "core",
    tags: ["Orange", "Light", "Cozy"],
    theme: {
      primary: "#c66b2f",
      secondary: "#d69366",
      accent: "#ad581f",
      background: "#f8ebe2",
      surface: "#eedbce",
      surfaceSecondary: "#e3caba",
      text: "#2e1c0f",
      textSecondary: "#845739",
      border: "#c6a28b",
      glow: "#e79c6a",
      shadow: "#b8a599",
      success: "#288a59",
      warning: "#957323",
      error: "#ba2c37",
    },
  },
  // Pale aqua lagoon with deep teal accents.
  {
    id: "lagoon",
    name: "Lagoon",
    collection: "core",
    tags: ["Blue", "Light", "Cozy"],
    theme: {
      primary: "#2fc6c6",
      secondary: "#66d6d6",
      accent: "#188b8b",
      background: "#e2f8f8",
      surface: "#ceeeee",
      surfaceSecondary: "#bae3e3",
      text: "#0f2e2e",
      textSecondary: "#317272",
      border: "#8bc6c6",
      glow: "#6ae7e7",
      shadow: "#99b8b8",
      success: "#298e5c",
      warning: "#9d7925",
      error: "#ba2c37",
    },
  },
  // Soft lemon ground with amber accents.
  {
    id: "lemonade",
    name: "Lemonade",
    collection: "core",
    tags: ["Orange", "Light", "Fantasy"],
    theme: {
      primary: "#c6a82f",
      secondary: "#d6c066",
      accent: "#937b1a",
      background: "#f8f4e2",
      surface: "#eee7ce",
      surfaceSecondary: "#e3dbba",
      text: "#2e280f",
      textSecondary: "#726531",
      border: "#c6ba8b",
      glow: "#e7ce6a",
      shadow: "#b8b299",
      success: "#298e5c",
      warning: "#9d7925",
      error: "#ba2c37",
    },
  },
  // Light coral sorbet with deep coral accents.
  {
    id: "sorbet",
    name: "Sorbet",
    collection: "core",
    tags: ["Pink", "Orange", "Light"],
    theme: {
      primary: "#c6432f",
      secondary: "#d67566",
      accent: "#ad321f",
      background: "#f8e5e2",
      surface: "#eed2ce",
      surfaceSecondary: "#e3c0ba",
      text: "#2e130f",
      textSecondary: "#8f483d",
      border: "#c6938b",
      glow: "#e77b6a",
      shadow: "#b89d99",
      success: "#268254",
      warning: "#916f22",
      error: "#ba2c37",
    },
  },


  // Soft sky blue on deep navy — light blue as the lead, matched by paler blues.
  {
    id: "sky",
    name: "Sky",
    collection: "core",
    tags: ["Blue", "Cozy", "Nordic"],
    theme: {
      primary: "#6fb7f0",
      secondary: "#9ad0f5",
      accent: "#cfe8ff",
      background: "#08111f",
      text: "#eef6ff",
      border: "#3a556f",
      glow: "#8fc7f7",
      shadow: "#05080d",
    },
  },
  // Baby pink, analogous rose and pale petal — a soft, matched pink family.
  {
    id: "blush",
    name: "Blush",
    collection: "core",
    tags: ["Pink", "Cozy", "Anime"],
    theme: {
      primary: "#f2a3c0",
      secondary: "#f7bfd4",
      accent: "#ffe0ec",
      background: "#170b11",
      text: "#fdeef4",
      border: "#6a4756",
      glow: "#f6b6cf",
      shadow: "#0c0508",
    },
  },
  // Fresh mint with lighter mint and pale seafoam — cool and cohesive.
  {
    id: "mint",
    name: "Mint",
    collection: "core",
    tags: ["Green", "Cozy", "Anime"],
    theme: {
      primary: "#7fe0c2",
      secondary: "#a6ecd6",
      accent: "#d1f5e8",
      background: "#06130f",
      text: "#ecfdf6",
      border: "#34604f",
      glow: "#93e8cd",
      shadow: "#05090a",
    },
  },
  // Gentle lavender graded to pale lilac — a calm, matched purple set.
  {
    id: "lilac",
    name: "Lilac",
    collection: "core",
    tags: ["Purple", "Cozy", "Anime"],
    theme: {
      primary: "#b3a2e6",
      secondary: "#c9bbf0",
      accent: "#e6dbf8",
      background: "#100b1b",
      text: "#f3eefb",
      border: "#574b70",
      glow: "#c3b2f0",
      shadow: "#07050b",
    },
  },
  // Warm peach and apricot fading to cream — a soft, harmonious warm palette.
  {
    id: "peach",
    name: "Peach",
    collection: "core",
    tags: ["Orange", "Cozy", "Anime"],
    theme: {
      primary: "#f4a985",
      secondary: "#f8c4a3",
      accent: "#ffdcc4",
      background: "#170c07",
      text: "#fdefe6",
      border: "#6a4c3a",
      glow: "#f6b895",
      shadow: "#0c0603",
    },
  },
  // Soft aqua/turquoise with lighter tints — a matched teal family.
  {
    id: "aqua",
    name: "Aqua",
    collection: "core",
    tags: ["Blue", "Green", "Cozy"],
    theme: {
      primary: "#6ad6d2",
      secondary: "#94e4df",
      accent: "#c2f1ee",
      background: "#061313",
      text: "#e8fdfb",
      border: "#33605d",
      glow: "#86e0dc",
      shadow: "#04090a",
    },
  },
  // Buttery yellow through soft lemon to cream — warm and coordinated.
  {
    id: "butter",
    name: "Butter",
    collection: "core",
    tags: ["Orange", "Cozy", "Fantasy"],
    theme: {
      primary: "#f0d47a",
      secondary: "#f5e4a2",
      accent: "#fbf1cf",
      background: "#141005",
      text: "#fdf9ea",
      border: "#6a5c33",
      glow: "#f4dd90",
      shadow: "#0a0803",
    },
  },
  // Soft coral and salmon with a pale rose — a matched warm-pink palette.
  {
    id: "coral",
    name: "Coral",
    collection: "core",
    tags: ["Pink", "Orange", "Cozy"],
    theme: {
      primary: "#f58a86",
      secondary: "#f7a9a6",
      accent: "#ffd0ce",
      background: "#180a0a",
      text: "#fdecec",
      border: "#6a4040",
      glow: "#f6a09c",
      shadow: "#0c0505",
    },
  },


  /* -------------------------------- Pride -------------------------------- */
  // The pride launch packs. Flag colours are harmonised per the design rules:
  // softened saturation, neutral backgrounds, text/accent pairs that pass the
  // contrast gate. Every one is a complete pack across the pride family.
  {
    id: "progress-pride",
    name: "Progress Pride",
    collection: "pride",
    subStyle: "Progress Pride",
    // The Progress flag's chevron colours joined onto the classic six — the
    // full inclusive stripe set, not a three-colour abbreviation.
    flag: ["#FFFFFF", "#F5A9B8", "#5BCEFA", "#613915", "#141414", "#E40303", "#FF8C00", "#FFED00", "#008026", "#24408E", "#732982"],
    tags: ["RGB", "Dark"],
    theme: {
      primary: "#d9486b",
      secondary: "#76c5e5",
      accent: "#f2b33d",
      accentSecondary: "#66bb7a",
      background: "#130e1d",
      text: "#f7f7fa",
      border: "#6f6a78",
      glow: "#e8875f",
      shadow: "#000000",
      success: "#66bb7a",
      warning: "#f5cc80",
      error: "#de7388",
    },
  },
  {
    id: "rainbow-spectrum",
    flag: ["#E40303", "#FF8C00", "#FFED00", "#008026", "#24408E", "#732982"],
    name: "Rainbow Spectrum",
    collection: "pride",
    subStyle: "Classic Pride",
    tags: ["RGB", "Dark"],
    theme: {
      primary: "#e05a5a",
      secondary: "#4d9be6",
      accent: "#f5b942",
      accentSecondary: "#66bb7a",
      background: "#140e1f",
      text: "#f8f7fa",
      border: "#6e6a78",
      glow: "#f08e5a",
      shadow: "#000000",
      success: "#66bb7a",
      warning: "#f5b942",
      error: "#e36d6d",
    },
  },
  {
    id: "crystal-pride",
    flag: ["#CDEBF7", "#DDE7FA", "#EAF4FB", "#E7DCF6", "#F4D9EC", "#CFF0E8"],
    name: "Crystal Pride",
    collection: "pride",
    subStyle: "Crystal Pride",
    tags: ["Blue", "Light", "Minimal"],
    theme: {
      primary: "#8fd0e8",
      secondary: "#c9b8f0",
      accent: "#e8f4fa",
      accentSecondary: "#f0c9e4",
      background: "#0e1218",
      text: "#f4fafd",
      border: "#3d4e59",
      glow: "#b8e4f5",
      shadow: "#000000",
    },
  },
  {
    id: "pastel-dream",
    flag: ["#F7B7B7", "#F9D9A9", "#FBF3B9", "#BFE8C5", "#B7D4F5", "#D8C4EE"],
    name: "Pastel Dream",
    collection: "pride",
    subStyle: "Pastel Pride",
    tags: ["Pink", "Cozy"],
    theme: {
      error: "#d37260",
      primary: "#f5b8d0",
      secondary: "#b8ccf5",
      accent: "#c9f0dc",
      accentSecondary: "#e8d8f5",
      background: "#141219",
      text: "#fdf8fb",
      border: "#4a4453",
      glow: "#f5d8e8",
      shadow: "#000000",
    },
  },
  {
    id: "neon-celebration",
    flag: ["#FF2D55", "#FF9500", "#FFE800", "#2DFF6A", "#2D9BFF", "#B14DFF"],
    name: "Neon Celebration",
    collection: "pride",
    subStyle: "Neon Pride",
    tags: ["Neon", "Pink", "RGB", "Dark"],
    theme: {
      warning: "#ffedc2",
      success: "#69d387",
      error: "#f0603f",
      primary: "#ff4d8f",
      secondary: "#29d9c2",
      accent: "#ffd23f",
      accentSecondary: "#8c5bff",
      background: "#0f0a16",
      text: "#fff7fb",
      border: "#7a4fa0",
      glow: "#ff7ab8",
      shadow: "#000000",
    },
  },
  {
    id: "trans-aurora",
    flag: ["#5BCEFA", "#F5A9B8", "#FFFFFF", "#F5A9B8", "#5BCEFA"],
    name: "Trans Aurora",
    collection: "pride",
    subStyle: "Trans Pride",
    tags: ["Blue", "Pink", "Cozy"],
    theme: {
      primary: "#6fbee6",
      secondary: "#f0a3c2",
      accent: "#7fe8d8",
      accentSecondary: "#f6cdde",
      background: "#10141c",
      text: "#f6fafd",
      border: "#5e7386",
      glow: "#8cd9f0",
      shadow: "#000000",
    },
  },
  {
    id: "gay-aurora",
    flag: ["#078D70", "#26CEAA", "#98E8C1", "#FFFFFF", "#7BADE2", "#5049CC", "#3D1A78"],
    name: "Gay Pride",
    collection: "pride",
    subStyle: "Gay Pride",
    tags: ["Green", "Blue", "Cozy"],
    theme: {
      primary: "#26CEAA",
      secondary: "#7BADE2",
      accent: "#26CEAA",
      accentSecondary: "#5049CC",
      background: "#0a1418",
      text: "#eefbf7",
      border: "#3a5a58",
      glow: "#4fe0c0",
      shadow: "#000000",
    },
  },
  {
    id: "galaxy-pride",
    flag: ["#FF5E8A", "#FFA75E", "#FFE85E", "#5EE0A0", "#5E9EFF", "#A05EFF"],
    name: "Galaxy Pride",
    collection: "pride",
    subStyle: "Cosmic Pride",
    tags: ["Purple", "Sci-Fi", "Dark"],
    theme: {
      warning: "#d9a63f",
      error: "#db5f4b",
      primary: "#9a82eb",
      secondary: "#e65ba8",
      accent: "#5bc8e6",
      accentSecondary: "#e8d6a8",
      background: "#100e1c",
      text: "#f5f2fd",
      border: "#6a5b96",
      glow: "#9c7af0",
      shadow: "#000000",
    },
  },
  {
    id: "soft-bloom",
    flag: ["#EFB7C4", "#F4D3A9", "#F7EFC0", "#C9E3B8", "#B9D4E8", "#DFC4E6"],
    name: "Soft Bloom",
    collection: "pride",
    subStyle: "Soft Pride",
    tags: ["Pink", "Green", "Cozy"],
    theme: {
      warning: "#d9a63f",
      primary: "#e89cb8",
      secondary: "#a8c8a0",
      accent: "#f5d9a8",
      accentSecondary: "#d8b8e8",
      background: "#131019",
      text: "#fbf6f4",
      border: "#8a7080",
      glow: "#f0bcd0",
      shadow: "#000000",
    },
  },
  {
    id: "modern-spectrum",
    flag: ["#C25B5B", "#C2915B", "#C2B85B", "#5BA06B", "#5B7BA6", "#8A5BA6"],
    name: "Modern Spectrum",
    collection: "pride",
    subStyle: "Dark Pride",
    tags: ["Dark", "Minimal"],
    theme: {
      error: "#f2919b",
      primary: "#5b5f6e",
      secondary: "#8a8fa3",
      accent: "#e0655f",
      accentSecondary: "#84a8cd",
      background: "#0f0f13",
      text: "#f2f2f5",
      border: "#4a4d5a",
      glow: "#e0655f",
      shadow: "#000000",
    },
  },
  {
    id: "minimal-pride",
    flag: ["#E40303", "#FF8C00", "#FFED00", "#008026", "#24408E", "#732982"],
    name: "Minimal Pride",
    collection: "pride",
    subStyle: "Minimal Pride",
    tags: ["Light", "Minimal"],
    theme: {
      textSecondary: "#5d5c63",
      primary: "#4a4e5c",
      secondary: "#7a6fa0",
      accent: "#c23a78",
      accentSecondary: "#3f7fc2",
      background: "#fbfafc",
      surface: "#ebe8f1",
      text: "#141318",
      border: "#b0abbc",
      glow: "#e8a8c8",
      shadow: "#8a8694",
    },
  },
  {
    id: "luxury-rainbow",
    flag: ["#B3273F", "#C2762B", "#C2A83B", "#2E7D5B", "#2B5AA6", "#6B3B8F"],
    name: "Luxury Rainbow",
    collection: "pride",
    subStyle: "Luxury Pride",
    tags: ["Dark", "Fantasy", "Orange"],
    theme: {
      warning: "#e08a35",
      success: "#6be8a7",
      primary: "#c9a44d",
      secondary: "#7a3b8f",
      accent: "#e8c46b",
      accentSecondary: "#3f8f7a",
      background: "#100d13",
      surface: "#1c1720",
      text: "#f7f2e8",
      border: "#6e5f4a",
      glow: "#e8c46b",
      shadow: "#000000",
    },
  },
  {
    id: "bi-pride",
    flag: ["#D60270", "#D60270", "#9B4F96", "#0038A8", "#0038A8"],
    name: "Bi Pride",
    collection: "pride",
    subStyle: "Bisexual",
    tags: ["Pink", "Purple", "Blue", "Dark"],
    theme: {
      error: "#e2574d",
      primary: "#c1437e",
      secondary: "#7a4e9e",
      accent: "#7f9ee8",
      background: "#110b16",
      text: "#f8f4fb",
      border: "#6e5580",
      glow: "#d06aa0",
      shadow: "#000000",
    },
  },
  {
    id: "lesbian-pride",
    flag: ["#D52D00", "#EF7627", "#FFFFFF", "#D162A4", "#A30262"],
    name: "Lesbian Pride",
    collection: "pride",
    subStyle: "Lesbian",
    tags: ["Orange", "Pink", "Dark"],
    theme: {
      primary: "#ef7627",
      secondary: "#d162a4",
      accent: "#ff9a5c",
      accentSecondary: "#e07ab0",
      background: "#131215",
      surface: "#1d1b20",
      text: "#faf6f4",
      border: "#5f5a5f",
      glow: "#f08a4e",
      shadow: "#000000",
    },
  },
  {
    id: "pan-pride",
    flag: ["#FF218C", "#FFD800", "#21B1FF"],
    name: "Pansexual Pride",
    collection: "pride",
    subStyle: "Pansexual",
    tags: ["Pink", "Blue", "Dark"],
    theme: {
      primary: "#ff5aa8",
      secondary: "#4db8ff",
      accent: "#ffcf3f",
      accentSecondary: "#4db8ff",
      background: "#131015",
      surface: "#1e1a20",
      text: "#fdf6fa",
      border: "#6a5a66",
      glow: "#ff7ab8",
      shadow: "#000000",
    },
  },
  {
    id: "nonbinary-pride",
    flag: ["#FCF434", "#FFFFFF", "#9C59D1", "#3a3a40"],
    name: "Nonbinary Pride",
    collection: "pride",
    subStyle: "Nonbinary",
    tags: ["Purple", "Dark", "Minimal"],
    theme: {
      primary: "#e6d94a",
      secondary: "#a86fd6",
      accent: "#f0e35a",
      accentSecondary: "#b98ee0",
      background: "#131316",
      surface: "#1d1d22",
      text: "#f8f7f2",
      border: "#605d64",
      glow: "#f0e878",
      shadow: "#000000",
    },
  },
  {
    id: "ace-pride",
    flag: ["#2e2e33", "#A3A3A3", "#FFFFFF", "#800080"],
    name: "Asexual Pride",
    collection: "pride",
    subStyle: "Asexual",
    tags: ["Purple", "Dark", "Minimal"],
    theme: {
      primary: "#a3a3a8",
      secondary: "#a05aa8",
      accent: "#b46abd",
      accentSecondary: "#c4c4c9",
      background: "#131315",
      surface: "#1d1d20",
      text: "#f6f6f8",
      border: "#5e5e64",
      glow: "#b06ab8",
      shadow: "#000000",
    },
  },
  {
    id: "genderfluid-pride",
    flag: ["#FF75A2", "#FFFFFF", "#BE18D6", "#2e2e40", "#333EBD"],
    name: "Genderfluid Pride",
    collection: "pride",
    subStyle: "Genderfluid",
    tags: ["Pink", "Purple", "Blue", "Dark"],
    theme: {
      primary: "#ff75a2",
      secondary: "#6a72d6",
      accent: "#c96ad6",
      accentSecondary: "#7f9ee8",
      background: "#131016",
      surface: "#1e1a22",
      text: "#fdf5fa",
      border: "#665a6e",
      glow: "#d06ad6",
      shadow: "#000000",
    },
  },
  {
    id: "agender-pride",
    flag: ["#2e2e33", "#B9B9B9", "#FFFFFF", "#B8F483", "#FFFFFF", "#B9B9B9", "#2e2e33"],
    name: "Agender Pride",
    collection: "pride",
    subStyle: "Agender",
    tags: ["Green", "Dark", "Minimal"],
    theme: {
      primary: "#b9b9be",
      secondary: "#a6df7c",
      accent: "#aee87a",
      accentSecondary: "#c9c9ce",
      background: "#131414",
      surface: "#1d1e1e",
      text: "#f6f8f4",
      border: "#5e6060",
      glow: "#cdf0a0",
      shadow: "#000000",
    },
  },
  {
    id: "aromantic-pride",
    flag: ["#3DA542", "#A7D379", "#FFFFFF", "#A9A9A9", "#2e2e33"],
    name: "Aromantic Pride",
    collection: "pride",
    subStyle: "Aromantic",
    tags: ["Green", "Dark", "Minimal"],
    theme: {
      primary: "#4faf53",
      secondary: "#a7d379",
      accent: "#63c268",
      accentSecondary: "#bfe090",
      background: "#121413",
      surface: "#1c1f1d",
      text: "#f5f8f4",
      border: "#5c625e",
      glow: "#7ad37a",
      shadow: "#000000",
    },
  },

  /* ----- Prism packs: the glass Prism Flag family, in a light and a dark ----- */
  /* mode for the rainbow and the lesbian flag. The glass sheet disperses the   */
  /* `flag` colours into a shimmering prism spectrum; the theme stays neutral   */
  /* so the flag carries the identity. Ids are prefixed `prism-` so only the    */
  /* Prism Flag family expands across them.                                     */
  {
    id: "prism-rainbow",
    flag: ["#E40303", "#FF8C00", "#FFED00", "#008026", "#24408E", "#732982"],
    name: "Prism Rainbow",
    collection: "pride",
    subStyle: "Prism Rainbow",
    tags: ["RGB", "Neon", "Dark"],
    theme: {
      primary: "#8f7de0",
      secondary: "#e0655f",
      accent: "#f5b942",
      accentSecondary: "#66bb7a",
      background: "#120d1e",
      text: "#f7f5fb",
      border: "#645b7a",
      glow: "#a98cf0",
      shadow: "#000000",
      success: "#66bb7a",
      warning: "#f5b942",
      error: "#e36d6d",
    },
  },
  {
    id: "prism-rainbow-light",
    flag: ["#E40303", "#FF8C00", "#FFED00", "#008026", "#24408E", "#732982"],
    name: "Prism Rainbow Light",
    collection: "pride",
    subStyle: "Prism Rainbow",
    tags: ["RGB", "Light", "Minimal"],
    theme: {
      primary: "#5a4e8c",
      secondary: "#b5486a",
      accent: "#7b2fa0",
      accentSecondary: "#2f7a5a",
      background: "#f6f3fb",
      surface: "#e9e3f2",
      surfaceSecondary: "#ddd4ec",
      text: "#171420",
      textSecondary: "#4f4a63",
      border: "#b3aec6",
      glow: "#b892e0",
      shadow: "#9a94a8",
      success: "#278657",
      warning: "#8a6414",
      error: "#ba2c37",
    },
  },
  {
    id: "prism-lesbian",
    flag: ["#D52D00", "#EF7627", "#FFFFFF", "#D162A4", "#A30262"],
    name: "Prism Lesbian",
    collection: "pride",
    subStyle: "Prism Lesbian",
    tags: ["Orange", "Pink", "Dark"],
    theme: {
      primary: "#ef7627",
      secondary: "#d162a4",
      accent: "#ff9a5c",
      accentSecondary: "#e07ab0",
      // Neutral dark ground so the warm flag lines stay legible against it.
      background: "#131215",
      surface: "#1d1b20",
      text: "#faf6f4",
      border: "#5f5a5f",
      glow: "#f08a4e",
      shadow: "#000000",
      success: "#5fb98a",
      warning: "#f0a54e",
      error: "#e2574d",
    },
  },
  {
    id: "prism-lesbian-light",
    flag: ["#D52D00", "#EF7627", "#FFFFFF", "#D162A4", "#A30262"],
    name: "Prism Lesbian Light",
    collection: "pride",
    subStyle: "Prism Lesbian",
    tags: ["Orange", "Pink", "Light"],
    theme: {
      primary: "#c24e2a",
      secondary: "#b8477e",
      accent: "#a30262",
      accentSecondary: "#c25a2e",
      // Neutral light ground so the flag lines stay distinct against it.
      background: "#f5f3f4",
      surface: "#e8e3e5",
      surfaceSecondary: "#ddd6d9",
      text: "#1b1518",
      textSecondary: "#5f545a",
      border: "#c2b8bd",
      glow: "#ef9f80",
      shadow: "#9a8e88",
      success: "#278657",
      warning: "#9a6414",
      error: "#ba2c37",
    },
  },

  /* ----- More pride flags for the Prism / Frost / Plasma flag families. ----- */
  /* Each flies an authentic flag over a neutral near-black ground so its colour */
  /* lines read; pure black stripes are lifted to a near-black so they stay      */
  /* visible as a line. Ids are `prism-` so the flag families expand across them. */
  {
    id: "prism-trans",
    flag: ["#5BCEFA", "#F5A9B8", "#FFFFFF", "#F5A9B8", "#5BCEFA"],
    name: "Prism Trans",
    collection: "pride",
    subStyle: "Prism Trans",
    tags: ["Blue", "Pink", "Dark"],
    theme: {
      primary: "#6fbee6",
      secondary: "#f0a3c2",
      accent: "#7fd6ea",
      accentSecondary: "#f6cdde",
      background: "#121317",
      surface: "#1c1e24",
      text: "#f6f9fc",
      border: "#5c6470",
      glow: "#8cd9f0",
      shadow: "#000000",
    },
  },
  {
    id: "prism-trans-light",
    flag: ["#5BCEFA", "#F5A9B8", "#FFFFFF", "#F5A9B8", "#5BCEFA"],
    name: "Prism Trans Light",
    collection: "pride",
    subStyle: "Prism Trans",
    tags: ["Blue", "Pink", "Light"],
    theme: {
      primary: "#3a9fd0",
      secondary: "#d76a94",
      accent: "#2f8fc4",
      accentSecondary: "#c25a86",
      background: "#f2f6fb",
      surface: "#e6edf5",
      surfaceSecondary: "#d9e4f0",
      text: "#14181f",
      textSecondary: "#4a5560",
      border: "#b3c0cf",
      glow: "#7fc4e6",
      shadow: "#94a2b0",
    },
  },
  {
    id: "prism-gay",
    flag: ["#078D70", "#26CEAA", "#98E8C1", "#FFFFFF", "#7BADE2", "#5049CC", "#3D1A78"],
    name: "Prism Gay",
    collection: "pride",
    subStyle: "Prism Gay",
    tags: ["Green", "Blue", "Dark"],
    theme: {
      primary: "#26CEAA",
      secondary: "#7BADE2",
      accent: "#7fd6c8",
      accentSecondary: "#5049CC",
      background: "#101417",
      surface: "#1a1f22",
      text: "#eefbf7",
      border: "#3a5a58",
      glow: "#4fe0c0",
      shadow: "#000000",
    },
  },
  {
    id: "prism-gay-light",
    flag: ["#078D70", "#26CEAA", "#98E8C1", "#FFFFFF", "#7BADE2", "#5049CC", "#3D1A78"],
    name: "Prism Gay Light",
    collection: "pride",
    subStyle: "Prism Gay",
    tags: ["Green", "Blue", "Light"],
    theme: {
      primary: "#0a7d63",
      secondary: "#4a72c4",
      accent: "#0e8c70",
      accentSecondary: "#4038a8",
      background: "#eff7f4",
      surface: "#e1eee9",
      surfaceSecondary: "#d4e4de",
      text: "#0e1a16",
      textSecondary: "#45564f",
      border: "#aec7bf",
      glow: "#4fd0b0",
      shadow: "#8ea099",
    },
  },
  {
    id: "prism-bi",
    flag: ["#D60270", "#D60270", "#9B4F96", "#0038A8", "#0038A8"],
    name: "Prism Bisexual",
    collection: "pride",
    subStyle: "Prism Bisexual",
    tags: ["Pink", "Purple", "Blue", "Dark"],
    theme: {
      primary: "#d0417e",
      secondary: "#6a72d6",
      accent: "#c15aa6",
      accentSecondary: "#7f9ee8",
      background: "#131017",
      surface: "#1d1a24",
      text: "#f8f4fb",
      border: "#645a70",
      glow: "#d06aa0",
      shadow: "#000000",
    },
  },
  {
    id: "prism-bi-light",
    flag: ["#D60270", "#D60270", "#9B4F96", "#0038A8", "#0038A8"],
    name: "Prism Bisexual Light",
    collection: "pride",
    subStyle: "Prism Bisexual",
    tags: ["Pink", "Purple", "Blue", "Light"],
    theme: {
      primary: "#c00263",
      secondary: "#0038A8",
      accent: "#8f2f80",
      accentSecondary: "#2f52c0",
      background: "#f7f2f6",
      surface: "#ece2ea",
      surfaceSecondary: "#e0d4df",
      text: "#1a1018",
      textSecondary: "#574a54",
      border: "#c2b2bd",
      glow: "#d06aa0",
      shadow: "#9a8a94",
    },
  },
  {
    id: "prism-pan",
    flag: ["#FF218C", "#FFD800", "#21B1FF"],
    name: "Prism Pansexual",
    collection: "pride",
    subStyle: "Prism Pansexual",
    tags: ["Pink", "Blue", "Dark"],
    theme: {
      primary: "#ff5aa8",
      secondary: "#4db8ff",
      accent: "#ffcf3f",
      accentSecondary: "#4db8ff",
      background: "#131015",
      surface: "#1e1a20",
      text: "#fdf6fa",
      border: "#6a5a66",
      glow: "#ff7ab8",
      shadow: "#000000",
    },
  },
  {
    id: "prism-pan-light",
    flag: ["#FF218C", "#FFD800", "#21B1FF"],
    name: "Prism Pansexual Light",
    collection: "pride",
    subStyle: "Prism Pansexual",
    tags: ["Pink", "Blue", "Light"],
    theme: {
      primary: "#e0186f",
      secondary: "#1088c4",
      accent: "#c78f00",
      accentSecondary: "#1088c4",
      background: "#fbf3f7",
      surface: "#f0e4ec",
      surfaceSecondary: "#e4d6df",
      text: "#1a1016",
      textSecondary: "#574a52",
      border: "#c8b4c0",
      glow: "#ff7ab8",
      shadow: "#9c8c94",
    },
  },
  {
    id: "prism-nonbinary",
    flag: ["#FCF434", "#FFFFFF", "#9C59D1", "#3a3a40"],
    name: "Prism Nonbinary",
    collection: "pride",
    subStyle: "Prism Nonbinary",
    tags: ["Purple", "Dark", "Minimal"],
    theme: {
      primary: "#e6d94a",
      secondary: "#a86fd6",
      accent: "#f0e35a",
      accentSecondary: "#b98ee0",
      background: "#131316",
      surface: "#1d1d22",
      text: "#f8f7f2",
      border: "#605d64",
      glow: "#f0e878",
      shadow: "#000000",
    },
  },
  {
    id: "prism-nonbinary-light",
    flag: ["#FCF434", "#FFFFFF", "#9C59D1", "#3a3a40"],
    name: "Prism Nonbinary Light",
    collection: "pride",
    subStyle: "Prism Nonbinary",
    tags: ["Purple", "Light", "Minimal"],
    theme: {
      primary: "#b89a10",
      secondary: "#7b3fb0",
      accent: "#8a4fc0",
      accentSecondary: "#5c5c64",
      background: "#f6f4f0",
      surface: "#eae7e0",
      surfaceSecondary: "#ded9d0",
      text: "#16150f",
      textSecondary: "#524f45",
      border: "#c0bbae",
      glow: "#c9a83a",
      shadow: "#999488",
    },
  },
  {
    id: "prism-ace",
    flag: ["#2e2e33", "#A3A3A3", "#FFFFFF", "#800080"],
    name: "Prism Asexual",
    collection: "pride",
    subStyle: "Prism Asexual",
    tags: ["Purple", "Dark", "Minimal"],
    theme: {
      primary: "#a3a3a8",
      secondary: "#a05aa8",
      accent: "#b46abd",
      accentSecondary: "#c4c4c9",
      background: "#131315",
      surface: "#1d1d20",
      text: "#f6f6f8",
      border: "#5e5e64",
      glow: "#b06ab8",
      shadow: "#000000",
    },
  },
  {
    id: "prism-ace-light",
    flag: ["#2e2e33", "#A3A3A3", "#FFFFFF", "#800080"],
    name: "Prism Asexual Light",
    collection: "pride",
    subStyle: "Prism Asexual",
    tags: ["Purple", "Light", "Minimal"],
    theme: {
      primary: "#5c5c63",
      secondary: "#800080",
      accent: "#8f2f9c",
      accentSecondary: "#6e6e75",
      background: "#f4f4f6",
      surface: "#e8e8ec",
      surfaceSecondary: "#dcdce2",
      text: "#151519",
      textSecondary: "#4f4f57",
      border: "#bcbcc4",
      glow: "#a05aa8",
      shadow: "#96969e",
    },
  },
  {
    id: "prism-genderfluid",
    flag: ["#FF75A2", "#FFFFFF", "#BE18D6", "#2e2e40", "#333EBD"],
    name: "Prism Genderfluid",
    collection: "pride",
    subStyle: "Prism Genderfluid",
    tags: ["Pink", "Purple", "Blue", "Dark"],
    theme: {
      primary: "#ff75a2",
      secondary: "#6a72d6",
      accent: "#c96ad6",
      accentSecondary: "#7f9ee8",
      background: "#131016",
      surface: "#1e1a22",
      text: "#fdf5fa",
      border: "#665a6e",
      glow: "#d06ad6",
      shadow: "#000000",
    },
  },
  {
    id: "prism-genderfluid-light",
    flag: ["#FF75A2", "#FFFFFF", "#BE18D6", "#2e2e40", "#333EBD"],
    name: "Prism Genderfluid Light",
    collection: "pride",
    subStyle: "Prism Genderfluid",
    tags: ["Pink", "Purple", "Blue", "Light"],
    theme: {
      primary: "#e0326e",
      secondary: "#2f38b0",
      accent: "#a020c0",
      accentSecondary: "#3a44c0",
      background: "#f8f2f7",
      surface: "#ece2ec",
      surfaceSecondary: "#e0d4e2",
      text: "#170f18",
      textSecondary: "#544a56",
      border: "#c4b4c6",
      glow: "#d06ad6",
      shadow: "#9a8a9c",
    },
  },
  {
    id: "prism-agender",
    flag: ["#2e2e33", "#B9B9B9", "#FFFFFF", "#B8F483", "#FFFFFF", "#B9B9B9", "#2e2e33"],
    name: "Prism Agender",
    collection: "pride",
    subStyle: "Prism Agender",
    tags: ["Green", "Dark", "Minimal"],
    theme: {
      primary: "#b9b9be",
      secondary: "#a6df7c",
      accent: "#aee87a",
      accentSecondary: "#c9c9ce",
      background: "#131414",
      surface: "#1d1e1e",
      text: "#f6f8f4",
      border: "#5e6060",
      glow: "#cdf0a0",
      shadow: "#000000",
    },
  },
  {
    id: "prism-aromantic",
    flag: ["#3DA542", "#A7D379", "#FFFFFF", "#A9A9A9", "#2e2e33"],
    name: "Prism Aromantic",
    collection: "pride",
    subStyle: "Prism Aromantic",
    tags: ["Green", "Dark", "Minimal"],
    theme: {
      primary: "#4faf53",
      secondary: "#a7d379",
      accent: "#63c268",
      accentSecondary: "#bfe090",
      background: "#121413",
      surface: "#1c1f1d",
      text: "#f5f8f4",
      border: "#5c625e",
      glow: "#7ad37a",
      shadow: "#000000",
    },
  },

  /* ----- Abstract packs: near-black identity palettes for the Riso Concrete ----- */
  /* and Aurora Silk families. Ids are `abstract-` so they expand only across    */
  /* those families, not every core design.                                      */
  {
    id: "abstract-riso-red",
    name: "Riso Red",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Minimal", "Red", "Dark"],
    theme: {
      primary: "#e8e3d8",
      secondary: "#5a2a86",
      accent: "#d81f2b",
      accentSecondary: "#7a3caa",
      background: "#0b0910",
      surface: "#161219",
      surfaceSecondary: "#201b24",
      text: "#f2ede3",
      textSecondary: "#9a9188",
      border: "#453f50",
      glow: "#d81f2b",
      shadow: "#000000",
    },
  },
  {
    id: "abstract-riso-red-light",
    name: "Riso Red Light",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Minimal", "Red", "Light"],
    theme: {
      primary: "#3a3028",
      secondary: "#5a2a86",
      accent: "#d81f2b",
      accentSecondary: "#7a3caa",
      background: "#f1ecdf",
      surface: "#e6e0cf",
      surfaceSecondary: "#dad3bf",
      text: "#1b1710",
      textSecondary: "#5f5648",
      border: "#c6bba4",
      glow: "#e0554f",
      shadow: "#9a9078",
    },
  },
  {
    id: "abstract-aurora",
    name: "Aurora Violet",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Minimal", "Purple", "Dark"],
    theme: {
      primary: "#6b2ce0",
      secondary: "#b03cf0",
      accent: "#ff2a3c",
      accentSecondary: "#8a0f1e",
      background: "#0b0910",
      surface: "#16101c",
      surfaceSecondary: "#1c1622",
      text: "#f5f1f6",
      textSecondary: "#b8b0c0",
      border: "#443c50",
      glow: "#ff3350",
      shadow: "#000000",
    },
  },
  {
    id: "abstract-aurora-light",
    name: "Aurora Violet Light",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Minimal", "Purple", "Light"],
    theme: {
      primary: "#7b3ce8",
      secondary: "#a24cea",
      accent: "#d81f2b",
      accentSecondary: "#a01020",
      background: "#f4f1fa",
      surface: "#e9e3f2",
      surfaceSecondary: "#ddd5ea",
      text: "#171320",
      textSecondary: "#544c62",
      border: "#c2b8d4",
      glow: "#b06ae8",
      shadow: "#9a92a8",
    },
  },
  {
    id: "abstract-vanguard-red",
    name: "Vanguard Red",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Red", "Minimal", "Esports", "Dark"],
    theme: {
      primary: "#c90000",
      secondary: "#9a9a9a",
      accent: "#c90000",
      background: "#353535",
      surface: "#2b2b2b",
      surfaceSecondary: "#262626",
      text: "#f5f5f5",
      textSecondary: "#cfcfcf",
      border: "#c90000",
      glow: "#c90000",
      shadow: "#000000",
    },
  },
  {
    id: "abstract-vanguard-purple",
    name: "Vanguard Purple",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Purple", "Minimal", "Esports", "Dark"],
    theme: {
      primary: "#8b2fd6",
      secondary: "#9a9a9a",
      accent: "#8b2fd6",
      background: "#353535",
      surface: "#2b2b2b",
      surfaceSecondary: "#262626",
      text: "#f5f5f5",
      textSecondary: "#cfcfcf",
      border: "#8b2fd6",
      glow: "#8b2fd6",
      shadow: "#000000",
    },
  },
  {
    id: "abstract-vanguard-blue",
    name: "Vanguard Blue",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Blue", "Minimal", "Esports", "Dark"],
    theme: {
      primary: "#1f6feb",
      secondary: "#9a9a9a",
      accent: "#1f6feb",
      background: "#353535",
      surface: "#2b2b2b",
      surfaceSecondary: "#262626",
      text: "#f5f5f5",
      textSecondary: "#cfcfcf",
      border: "#1f6feb",
      glow: "#1f6feb",
      shadow: "#000000",
    },
  },
  {
    id: "abstract-vanguard-red-light",
    name: "Vanguard Red Light",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Red", "Minimal", "Esports", "Light"],
    theme: {
      primary: "#c90000",
      secondary: "#6f6f6f",
      accent: "#c90000",
      background: "#e9e9e9",
      surface: "#f2f2f2",
      surfaceSecondary: "#dedede",
      text: "#161616",
      textSecondary: "#4a4a4a",
      border: "#c90000",
      glow: "#e83a3a",
      shadow: "#a4a4a4",
    },
  },
  {
    id: "abstract-vanguard-purple-light",
    name: "Vanguard Purple Light",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Purple", "Minimal", "Esports", "Light"],
    theme: {
      primary: "#8b2fd6",
      secondary: "#6f6f6f",
      accent: "#8b2fd6",
      background: "#eae8ee",
      surface: "#f3f1f6",
      surfaceSecondary: "#ded9e6",
      text: "#161418",
      textSecondary: "#4a4652",
      border: "#8b2fd6",
      glow: "#a24ce0",
      shadow: "#a29aa8",
    },
  },
  {
    id: "abstract-vanguard-blue-light",
    name: "Vanguard Blue Light",
    collection: "core",
    subStyle: "Abstract",
    tags: ["Blue", "Minimal", "Esports", "Light"],
    theme: {
      primary: "#1f6feb",
      secondary: "#6f6f6f",
      accent: "#1f6feb",
      background: "#e8ebef",
      surface: "#f1f3f6",
      surfaceSecondary: "#dbe0e6",
      text: "#14171b",
      textSecondary: "#454b52",
      border: "#1f6feb",
      glow: "#4a90f0",
      shadow: "#9aa2ac",
    },
  },
];

/** Public palettes: core tokens authored above, the rest derived, plus the
    synthetic palettes of any baked-in custom designs. */
export const PALETTES: Palette[] = [
  ...DEFS.map((def) => ({ ...def, theme: completeTheme(def.theme) })),
  ...CUSTOM_PALETTES,
];

export const DEFAULT_PALETTE_ID = "purple-neon";

const BY_ID = new Map(PALETTES.map((p) => [p.id, p]));

export function getPalette(id: string): Palette {
  return BY_ID.get(id) ?? PALETTES[0];
}

export function paletteTags(id: string): StyleTag[] {
  return getPalette(id).tags as StyleTag[];
}

// Abstract identity palettes ride the core collection but only expand across the
// abstract families, so keep them out of the general core expansion.
export const CORE_PALETTES = PALETTES.filter(
  (p) => p.collection === "core" && !p.id.startsWith("abstract-") && !p.id.startsWith("custom-"),
);
export const ABSTRACT_PALETTES = PALETTES.filter((p) => p.id.startsWith("abstract-"));
export const GOTHIC_PALETTES = PALETTES.filter((p) => p.collection === "gothic");
// Pride splits into the classic pride packs and the glass Prism Flag packs: the
// `prism-` palettes only make sense under the Prism Flag family (they disperse
// into its glass sheet), so they expand separately, not across every design.
export const PRIDE_PALETTES = PALETTES.filter(
  (p) => p.collection === "pride" && !p.id.startsWith("prism-"),
);
export const PRISM_PALETTES = PALETTES.filter((p) => p.id.startsWith("prism-"));
